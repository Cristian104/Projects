...
from app.modules.gym.models import GymRoutine, GymExercise, GymProgram, GymExerciseLibrary, GymLog
...
@gym_bp.route('/import_pdf', methods=['POST'])
@login_required
def import_pdf():
    """Parse a coach's PDF with Gemini and create a gym program (optionally + nutrition plan)."""
    import json
    from app.modules.nutrition.routes import _parse_pdf_with_gemini

    f = request.files.get('pdf_file')
    program_name = request.form.get('program_name', 'Imported Program')
    also_nutrition = request.form.get('also_nutrition') == 'on'
    ...
    # Save PDF
    upload_dir = os.path.join(current_app.root_path, '..', 'instance', 'uploads', 'gym_pdfs')
    os.makedirs(upload_dir, exist_ok=True)
    filename = secure_filename(f'{current_user.id}_{datetime.now().timestamp()}_{f.filename}')
    ...
    # Parse with Gemini
    try:
        parsed = _parse_pdf_with_gemini(text, program_name)
    except Exception as e:
        flash(f'AI parsing failed: {e}', 'error')
        return redirect(url_for('gym.programs'))

    gym_data = parsed.get('gym_program')
    if not gym_data:
        flash('No gym program found in this PDF.', 'error')
        return redirect(url_for('gym.programs'))

    # Create gym program
    GymProgram.query.filter_by(user_id=current_user.id).update({'is_active': False})
    prog = GymProgram(user_id=current_user.id, name=gym_data.get('name', program_name), is_active=True)
    db.session.add(prog)
    db.session.flush()
    ...
    # Optionally create nutrition plan too
    if also_nutrition:
        from app.modules.nutrition.routes import _create_records_from_parsed
        try:
            _create_records_from_parsed(parsed, program_name, filename, also_gym=False)
        except Exception:
            pass  # Gym program already saved; nutrition failure is non-fatal

    flash(f'Program "{prog.name}" imported successfully!', 'success')
...
@gym_bp.route('/routine/<int:routine_id>/workout')
@login_required
def workout_session(routine_id):
    routine = GymRoutine.query.get_or_404(routine_id)
    session_id = str(uuid.uuid4())

    # Get previous best for each exercise
    prev_bests = {}
    for ex in routine.exercises:
        best = GymLog.query.filter_by(
            user_id=current_user.id, exercise_id=ex.id
        ).order_by(GymLog.weight_used.desc(), GymLog.reps_done.desc()).first()
        if best:
            prev_bests[ex.id] = {'weight': best.weight_used, 'reps': best.reps_done}

    return render_template('gym/workout.html', routine=routine,
                           session_id=session_id, prev_bests=prev_bests)


@gym_bp.route('/api/log_set', methods=['POST'])
@login_required
def log_set():
    ...
    # Convert weight to float or None
    try:
        weight = float(weight) if weight not in (None, '', 0, '0') else None
    ...
    # PR detection
    prev_best = GymLog.query.filter_by(
    ...
    if weight is not None and reps is not None:
        if prev_best is None:
            is_pr = True
        elif weight > (prev_best.weight_used or 0):
            is_pr = True
        elif weight == (prev_best.weight_used or 0) and reps > (prev_best.reps_done or 0):
            is_pr = True
    ...
    return jsonify({'success': True, 'log_id': log_entry.id, 'is_pr': is_pr})


@gym_bp.route('/api/delete_log_set/<int:log_id>', methods=['DELETE'])
@login_required
def delete_log_set(log_id):
    log = GymLog.query.get_or_404(log_id)
...
@gym_bp.route('/routine/<int:routine_id>/complete', methods=['POST'])
@login_required
def complete_workout(routine_id):
    current = current_user.current_gym_day or 1
    current_user.current_gym_day = 1 if current >= 7 else current + 1
    db.session.commit()
    return jsonify({'success': True, 'next_day': current_user.current_gym_day})


@gym_bp.route('/exercise/<int:exercise_id>/progress')
@login_required
def exercise_progress(exercise_id):
    ...
    daily_max = db.session.query(
        GymLog.date,
        func.max(GymLog.weight_used).label('max_weight'),
        func.max(GymLog.reps_done).label('max_reps')
    ).filter_by(user_id=current_user.id, exercise_id=exercise_id
    ).group_by(GymLog.date).order_by(GymLog.date).all()

    # All-time PR logs
    pr_logs = GymLog.query.filter_by(
        user_id=current_user.id, exercise_id=exercise_id, is_personal_record=True
    ).order_by(GymLog.date.desc()).all()

    # Last 10 sessions (by session_id or date)
    recent_logs = GymLog.query.filter_by(
        user_id=current_user.id, exercise_id=exercise_id
    ).order_by(GymLog.date.desc(), GymLog.set_number.asc()).limit(30).all()

    chart_data = [{'date': str(r.date), 'weight': r.max_weight, 'reps': r.max_reps}
                  for r in daily_max]
    pr_dates = [str(p.date) for p in pr_logs]

    return render_template('gym/progress.html', exercise=exercise,
                           chart_data=chart_data, pr_dates=pr_dates,
